...
const test31 = (ft, q) => {
  ...
  console.log(`我有${qs} 个 ${ft}!`);
};

// 事实上，我们可以通过函数的默认参数来去掉变量 q。
// 请注意，所有的函数参数都可以有其默认值。举例来说，我们同样可以为 ft 赋予一个默认值：function test(ft = 'unknown', quantity = 1)
const test32 = (ft, q = 1) => { // 如果没有提供 q，默认为 1
  if (!ft) return;
  console.log(`我有${q} 个 ${ft}!`);
};
...
// 222222 object
// 那么如果 ft 是一个对象（Object）呢？我们还可以使用默认参数吗？
const test33 = (ft) => {
  // 如果有值，则打印出来
  if (ft && ft.name) {
    console.log(ft.name);
...
// 当水果名称属性存在时，我们希望将其打印出来，否则打印『unknown』。我们可以通过默认参数和解构赋值的方法来避免写出 ft && ft.name 这种条件
// 解构 —— 只得到 name 属性
// 默认参数为空对象 {}
const test34 = ({ name } = {}) => {
  console.log(name || 'unknown');
};
// 既然我们只需要 ft 的 name 属性，我们可以使用 {name} 来将其解构出来，之后我们就可以在代码中使用 name 变量来取代 ft.name
